...
import { Copy, Ruler, AlertTriangle, MessageCircle, Mail, ChevronDown, ChevronUp } from "lucide-react";
 ...
 * 默认折叠、点击展开（与 PCB 报价页一致）；尺寸默认跟随定位板编辑器、可手动修改（0 = 跟随）；
 * 独立数量；复用 PCB 板材逻辑；计价乘数 multiplier（默认 2.5）、加工费下限 minProcessFee（默认 300）；
 * 页面不显示成本构成（只显示单价/总价）；人工报价 Discord/Email 按钮 */
export default function PlatePricingSection({ plateSize = null }: PlatePricingSectionProps) {
  ...
  const handleCopy = async () => {
    ...
    const lines = [
      ...
      `定位板总价: ¥${quote.totalPrice.toFixed(2)}`,
